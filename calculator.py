...
def tokenize(expression):#Розбиває рядок на токени
    tokens = []
    i = 0
    n = len(expression)
    while i < n:
        ...
        if char in '+-*/()':
            ...
            num_str = ''
            while i < n and (expression[i].isdigit() or expression[i] == '.'):#поки бачимо цифри,крапку і додаємо їх.
                num_str += expression[i]
                i += 1
            tokens.append(float(num_str))#float() перетворює текст у число
        else:
            raise ValueError(f"Неприпустимий символ: {char}")
    return tokens


class Parser:#Parser — який аналізує математичний вираз.
    def __init__(self, tokens):
        self.tokens = tokens
        self.pos = 0

    def current(self):  #Який токен зараз перед нами?
    ...
    def parse_factor(self):
        ...
        if curr == '(':
            ...
            self.pos += 1
            return result
        if isinstance(curr, (int, float)):#isinstance() перевіряє тип даних
            self.pos += 1
            return curr
